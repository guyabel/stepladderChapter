...
read_un_totals <- function(s = NULL, col = NULL, col_name = NULL){
  read_excel(path = "./data-raw/undesa_pd_2019_migrant_stock_total_dataset.xlsx", 
             sheet = s, skip = 15, na = "..") %>%
    select({col}) %>%
    rename(name = 1, 
           country_code = 2) %>%
...
d1 <- read_un_totals(s = 3, col = c(2, 4:11), col_name = "pop")
...
d <- read_csv(file = "./data/stock_bilat.csv", guess_max = 1e5)
d4 <- d %>%
...
d6 <- d0 %>%
  ...
  mutate(pop = pop * 1e3, 
         disp_share = disp/pop)
write_csv(x = d6, path = "./data/stock_totals.csv")
...
##
d7 <- d6 %>%
  filter(country_code %in% c(903:905, 908, 909, 935),
         name != "OCEANIA") %>%
  select(-alpha3)

write_csv(x = d7, path = "./data/stock_totals_region.csv")